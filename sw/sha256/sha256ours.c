...
void SHA256Update(SHA256_CTX *ctx, uchar data[], uint len)
{
	for (uint i = 0; i < len; ++i) {
		...
		ctx->datalen++;
		if(ctx->datalen != i+1 )
			//printf("data len is %08x and i is %08x\n", ctx->datalen, i);
		//printf("data len is %08x and i is %08x\n", ctx->datalen, i);
		if (ctx->datalen == 64) {
...
{	
	...
	ptr = &secrets;
	printf("Address of secrets: %p\n", (void *)ptr);
    ptr = &secrets[0];
	printf("Address of secrets0: %p\n", (void *)ptr);
	ptr = &secrets[0][1];
	printf("Address of secrets01: %p\n", (void *)ptr);
	ptr = &secrets[1];
	printf("Address of secrets: %p\n", (void *)ptr);
	...
	unsigned int *d = (unsigned int*)0xf0030012;
	...
	*b = 0x000001234;
	...
	go = (unsigned int*)0x1;
	printf("after GO\n");
	*c = (unsigned int*)0xf0030010;
    *d = (unsigned int*)0xf0030012;
	printf("a is %x,  b is %x,  c is %x,  counter %x \n", *a, *b, *c, *counter);
	printf("adrees c is %x value at adress  c is  %x \n", c, *c);
	
	
	printf("a is %x,  b is %x,  c is %x\n", *a, *b, *c);
	printf("a is %x,  b is %x,  d is %x\n", *a, *b, *d);
